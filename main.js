...
function updateScoreDisplay() {
    ...
    scoreElement.forEach((scoreElement) => {
...
function checkAnswer() {
    ...
    let correctAnswerText = questions[currentQuestionIndex].correctAnswer.split("\n")[0]; // Extract correct answer before explanation
    let correctOption = questions[currentQuestionIndex].answers.find(answer => answer.includes(correctAnswerText));
...
document.querySelector(".quit").addEventListener("click", () => {
    if (confirm("Are you sure you want to quit the quiz?")) {
        ...
        document.querySelector(".score_container").style.display = "block";
    }
});
...
function endQuiz() {
    alert(`Quiz Over! Your final score is: ${userScore}`);
